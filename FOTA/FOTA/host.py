...
def Read_Data_From_Serial_Port(Command_Code):
    ...
    if(len(BL_ACK)):
        ...
        if(BL_ACK_Array[0] == 0xCD):
            ...
            elif(Command_Code == CBL_GET_VER_CMD):
                Process_CBL_GET_VER_CMD(Length_To_Follow)
...
def Process_CBL_GET_VER_CMD(Data_Len):
    ...
    print("\n   Bootloader Vendor ID : ", value[0])
    print("   Bootloader Version   : ", value[1], ".", value[2], ".", value[3])
...
def Process_CBL_MEM_WRITE_CMD(Data_Len):
    ...
    if(BL_Write_Status[0] == FLASH_PAYLOAD_WRITE_FAILED):
        ...
        print("\n   Write Status -> Write Successfully ")
...
def Decode_CBL_Command(Command):
    ...
    if (Command == 1):
        print("sector erase of flash from sector(2)of address (0x8008000) till last sector (sector 5) ")
        ...
        sleep(1.5)
        print("Write data into Flash memory of the MCU command")
        global Memory_Write_Is_Active
        global Memory_Write_All
        File_Total_Len = 0
...
while True:
    ...
    print("   CBL_FLASH_ERASE_And_Write          --> 1")
    
    
    CBL_Command = int(sys.argv[1])
